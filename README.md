...
piPipBoy — A Pip‑Boy‑style UI for Raspberry Pi 5

A retro, Fallout-inspired, Pip‑Boy-style user interface designed to run on Raspberry Pi 5 (Raspberry Pi OS / Debian Bookworm). This project provides a modular app framework (FileManager, Map, Environment, Clock, Radio, Update, Debug) with pluggable hardware interface drivers and a desktop Tk-based dev mode.
...
- Hardware support: SPI displays (ILI9486 style), GPIO buttons, rotary encoder (KY‑040), optional sensors (BME280, DS3231)
...
Acknowledgements
- Portions of this project build upon the `app` module from **piboy** by SirLefti (MIT). See https://github.com/SirLefti/piboy — MIT license notices are preserved in adapted files and credited in `CREDITS.md`.
